 ...
 */
        $parameter=array(
            ...
            "notifyUrl"      =>Request::instance()->domain().'/pay/notify/ZlfAliScan',//[必填]订单支付结果异步通知地址，用于接收订单支付结果通知，必须以http或https开头
            "returnUrl"      =>Request::instance()->domain().'/pay/page/ZlfAliScan',//[必填]订单支付结果同步跳转地址，用于同步跳转到商户页面，必须以http或https开头
            "subject"        =>$subject,//[必填]商品名称
            "openId"         =>"",
            "version"        =>"api_NoEncrypt",//接口版本号，值为pc_NoEncrypt时,则明天平台返回商户参数时，不进行RSA加密
            "payChannelId"   =>"0000000002",//支付渠道id
            "description"    =>"",
            "extra"          =>"",
            "expireMs"       =>"",
            "cpChannel"      =>"",
        ...
        if($response['respCode'] == '200') {
            parse_str($response['extra'], $extra_arr);
            $this->code    =0;
            $obj           =new \stdClass();
            $obj->pay_url  =$extra_arr['code_url'];
            $obj->content_type = 1;
            return $obj;
        } else {
            $this->code    =201;
