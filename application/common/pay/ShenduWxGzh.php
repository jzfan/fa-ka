...
class ShenduWxGzh extends Pay
{
    ...
    public function order($outTradeNo,$subject,$totalAmount)
    {
        $params = [
            ...
            'notify_url'   => Request::instance()->domain().'/pay/notify/ShenduWxGzh',
            'payment_code' => '8002',
            'out_trade_no' => $outTradeNo,
            'total_fee'    => $totalAmount,
            'body'         => $subject,
            'return_url'   => Request::instance()->domain().'/pay/page/ShenduWxGzh',
        ];
        ...
        if($response['state'] == '1') {
            ...
            if(Request::instance()->isMobile()) {
                if($this->is_weixin()) {
                    ...
                    $this->code    =0;
                    $obj           =new \stdClass();
                    $obj->pay_url  =url('index/pay/wx_jspay_page') . '?trade_no=' . $outTradeNo . '&url=' . base64_encode($response['data']['jump_url']);
                    $obj->content_type = 2;
                    return $obj;
                }
            } else {
