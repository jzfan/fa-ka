...
class DzJdScan extends Pay
{
    ...
    public function order($outTradeNo,$subject,$totalAmount)
    {
        ...
        $notify_url = Request::instance()->domain().'/pay/notify/DzJdScan';
        $input = new \JdScan($appid, $notify_url);
    ...
    public function notify_callback($params,$order)
    {
        ...
        if($result->GetSuccess($this->account->params->appsecret,false)) {
            ...
            if($order->total_price!=$amount){
                record_file_log('DzJdScan_notify_error','金额异常！'."\r\n".$order->trade_no."\r\n订单金额：{$order->total_price}，已支付：{$amount}");
                die('金额异常！');
            }
            // 流水号
            $order->transaction_id =$platformOrderNo;
            $this->completeOrder($order);
            record_file_log('DzJdScan_notify_success',$order->trade_no);
            echo 'ok';
            return true;
        }
        else {
