...
class KjAlipayScanImgPay extends Pay
{
    ...
    public function order($outTradeNo,$subject,$totalAmount)
    {
        $config = [
            ...
            'notify_url'        => Request::instance()->domain().'/pay/notify/KjAlipayScanImgPay',
        ...
        //请求获取支付地址
        $response = HttpService::post(trim($this->account->params->api_url, '/') . '/alipay/direct_code', $params, 30, [], $this->account->params->refer);
    ...
    public function notify_callback($params,$order)
    {
        if($params['status'] == 'Success'){
            ...
            if ($sign != $params["sign"]) {
                record_file_log('KjAlipayScanImgPay_notify_error','验签错误！'."\r\n".$order->trade_no);
                die('验签错误！');
            }
            // 金额异常检测
            $money=$params['amount'];
            if($order->total_price>$money){
                record_file_log('KjAlipayScanImgPay_notify_error','金额异常！'."\r\n".$order->trade_no."\r\n订单金额：{$order->total_price}，已支付：{$money}");
                die('金额异常！');
            }
            ...
            record_file_log('KjAlipayScanImgPay_notify_success',$order->trade_no);
            echo 'success';
            return true;
        }else if($params['status'] == 'Fail'){
            record_file_log('KjAlipayScanImgPay_notify_error','交易失败！'."\r\n".$order->trade_no);
            die('交易失败！');
        }
    }
     ...
     * @return String
     */
    protected function sign(Array $datas = [], String $key = "") : String
    {
